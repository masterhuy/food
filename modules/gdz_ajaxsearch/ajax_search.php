...
require_once(dirname(__FILE__).'/gdz_ajaxsearch.php');
...
if (Tools::strlen($search_key) >= 3) {
    if ($products = Product::searchByName((int) $context->language->id, $search_key)) {
        ...
        for ($i = 0; $i < $total_products; $i++) {
            if (($products[$i]['name']) && ($products[$i]['active'])) {
                ...
                if ((int)Configuration::get('GDZ_AJAXSEARCH_DESC_COUNT') < Tools::strlen($product->description_short[$context->language->id])) {
                    $pos = strpos($product->description_short[$context->language->id], ' ', (int)Configuration::get('GDZ_AJAXSEARCH_DESC_COUNT'));
                ...
                if ($count < Configuration::get('GDZ_AJAXSEARCH_COUNT')) {
    ...
    $context->smarty->assign(array(
        'show_image' => Configuration::get('GDZ_AJAXSEARCH_SHOW_IMAGE'),
        'show_desc' => Configuration::get('GDZ_AJAXSEARCH_SHOW_DESC'),
        'show_price' => Configuration::get('GDZ_AJAXSEARCH_SHOW_PRICE'),
        'link' => $context->link,
        'products' => $products,
