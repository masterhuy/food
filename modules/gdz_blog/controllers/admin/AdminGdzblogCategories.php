...
include_once(_PS_MODULE_DIR_.'gdz_blog/classes/gdzCategory.php');
class AdminGdzblogCategoriesController extends ModuleAdminController
{
    ...
    public function renderList()
    {
        ...
        if (Tools::isSubmit('CancelAddForm')) {
            Tools::redirectAdmin($this->context->link->getAdminLink('AdminGdzblogCategories', true).'&configure='.$this->name.'&tab_module='.$this->tab.'&module_name='.$this->name);
    ...
    private function _postProcess()
    {
        ...
        if (Tools::isSubmit('submitCategory')) {
            ...
            foreach ($languages as $language) {
                ...
                if (isset($_FILES['image_'.$language['id_lang']]) && isset($_FILES['image_'.$language['id_lang']]['tmp_name']) && !empty($_FILES['image_'.$language['id_lang']]['tmp_name']) && !empty($imagesize) && in_array(Tools::strtolower(Tools::substr(strrchr($imagesize['mime'], '/'), 1)), array('jpg', 'gif', 'jpeg', 'png')) && in_array($type, array('jpg', 'gif', 'jpeg', 'png'))) {
                    ...
                    gdz_blogHelper::createThumb(_PS_MODULE_DIR_.'/gdz_blog/views/img/', Tools::encrypt($_FILES['image_'.($language['id_lang'])]['name'].$salt).'.'.$type, $gdz_blog_setting['GDZBLOG_IMAGE_THUMB_WIDTH'], $gdz_blog_setting['GDZBLOG_IMAGE_THUMB_HEIGHT'], 'thumb_', 1);
                    //delete old img
            ...
            $item = new gdzCategory((int)Tools::getValue('delete_id_category'));
            $res = $item->delete();
            if (!$res) {
                $this->_html .= Tools::displayError('Could not delete');
            } else {
                Tools::redirectAdmin($this->context->link->getAdminLink('AdminGdzblogCategories', true).'&conf=1&configure='.$this->name.'&tab_module='.$this->tab.'&module_name='.$this->name);
            }
        } elseif (Tools::isSubmit('changeCategoryStatus') && Tools::isSubmit('status_id_category')) {
            $item = new gdzCategory((int)Tools::getValue('status_id_category'));
            if ($item->active == 0) {
                $item->active = 1;
            } else {
                $item->active = 0;
            ...
            if (!$res) {
                ...
                Tools::redirectAdmin($this->context->link->getAdminLink('AdminGdzblogCategories', true).'&conf=5&configure='.$this->name.'&tab_module='.$this->tab.'&module_name='.$this->name);
            }
        }
        ...
        if (count($errors)) {
            ...
            Tools::redirectAdmin($this->context->link->getAdminLink('AdminGdzblogCategories', true).'&conf=4&configure='.$this->name.'&tab_module='.$this->tab.'&module_name='.$this->name);
        }
    }
    ...
    public function categoryExists($id_category)
    {
        $req = 'SELECT hs.`category_id`
                FROM `'._DB_PREFIX_.'gdz_blog_categories` hs
    ...
    public function renderNavigation()
    {
        ...
        $html .= '<a class="btn btn-default" href="'.AdminController::$currentIndex.
            '&configure='.$this->name.'
                &token='.Tools::getAdminTokenLite('AdminGdzblogCategories').'" title="Back to Dashboard"><i class="icon-home"></i>Back to Dashboard</a>';
        $html .= '</div>';
        return $html;
    }
    ...
    {
        if (Tools::getValue('controller') != 'AdminGdzblogCategories' && Tools::getValue('configure') != $this->name) {
            return;
        }
