...
include_once(_PS_MODULE_DIR_.'gdz_blog/classes/gdzPost.php');
class AdminGdzblogPostController extends ModuleAdminController
{
    ...
    public function renderList()
    {
        ...
        if (Tools::isSubmit('CancelAddForm')) {
            Tools::redirectAdmin($this->context->link->getAdminLink('AdminGdzblogPost', true).'&configure='.$this->name.'&tab_module='.$this->tab.'&module_name='.$this->name);
    ...
    private function _postProcess()
    {
        ...
        if (Tools::isSubmit('submitPost')) {
            ...
            if (Tools::getValue('id_post')) {
                $item = new gdzPost((int)Tools::getValue('id_post'));
            ...
            foreach ($languages as $language) {
                ...
                if (!$item->alias[$language['id_lang']]) {
                    $item->alias[$language['id_lang']] = gdz_blogHelper::makeAlias($item->title[$language['id_lang']]);
                }
                ...
                if (isset($_FILES['image_'.$language['id_lang']]) && isset($_FILES['image_'.$language['id_lang']]['tmp_name']) && !empty($_FILES['image_'.$language['id_lang']]['tmp_name']) && !empty($imagesize) && in_array(Tools::strtolower(Tools::substr(strrchr($imagesize['mime'], '/'), 1)), array('jpg', 'gif', 'jpeg', 'png')) && in_array($type, array('jpg', 'gif', 'jpeg', 'png'))) {
                    ...
                    } elseif (!ImageManager::resize($temp_name, _PS_MODULE_DIR_.'/gdz_blog/views/img/'.Tools::encrypt($_FILES['image_'.$language['id_lang']]['name'].$salt).'.'.$type, null, null, $type)) {
                    ...
                    gdz_blogHelper::createThumb(_PS_MODULE_DIR_.'/gdz_blog/views/img/', Tools::encrypt($_FILES['image_'.($language['id_lang'])]['name'].$salt).'.'.$type, $gdz_blog_setting['GDZBLOG_IMAGE_WIDTH'], $gdz_blog_setting['GDZBLOG_IMAGE_HEIGHT'], 'resized_', 0);
                    gdz_blogHelper::createThumb(_PS_MODULE_DIR_.'/gdz_blog/views/img/', Tools::encrypt($_FILES['image_'.($language['id_lang'])]['name'].$salt).'.'.$type, $gdz_blog_setting['GDZBLOG_IMAGE_THUMB_WIDTH'], $gdz_blog_setting['GDZBLOG_IMAGE_THUMB_HEIGHT'], 'thumb_', 0);
                    //delete old img
                    $old_img = Tools::getValue('image_old_'.$language['id_lang']);
                    if ($old_img && file_exists(_PS_MODULE_DIR_.'/gdz_blog/views/img/'.$old_img)) {
                        @unlink(_PS_MODULE_DIR_.'/gdz_blog/views/img/'.$old_img);
                        @unlink(_PS_MODULE_DIR_.'/gdz_blog/views/img/resized_'.$old_img);
                        @unlink(_PS_MODULE_DIR_.'/gdz_blog/views/img/thumb_'.$old_img);
                    }
			...
			Tools::redirectAdmin($this->context->link->getAdminLink('AdminGdzblogPost', true).'&conf=3&configure='.$this->name.'&tab_module='.$this->tab.'&module_name='.$this->name);
        } elseif (Tools::isSubmit('delete_id_post')) {
            $item = new gdzPost((int)Tools::getValue('delete_id_post'));

            if ($item->delete() == false) {
                $this->_html .= Tools::displayError('Could not delete');
            } else {
                Tools::redirectAdmin($this->context->link->getAdminLink('AdminGdzblogPost', true).'&conf=1&configure='.$this->name.'&tab_module='.$this->tab.'&module_name='.$this->name);
            }
        } elseif (Tools::isSubmit('changePostStatus') && Tools::isSubmit('status_id_post')) {
            $item = new gdzPost((int)Tools::getValue('status_id_post'));
            if ($item->active == 0) {
                $item->active = 1;
            } else {
                $item->active = 0;
            ...
            if (!$res) {
                ...
                Tools::redirectAdmin($this->context->link->getAdminLink('AdminGdzblogPost', true).'&conf=5&configure='.$this->name.'&tab_module='.$this->tab.'&module_name='.$this->name);
            }
        }
        ...
        if (count($errors)) {
            ...
            Tools::redirectAdmin($this->context->link->getAdminLink('AdminGdzblogPost', true).'&conf=4&configure='.$this->name.'&tab_module='.$this->tab.'&module_name='.$this->name);
        }
    }
    ...
    public function postExists($id_post)
    {
        $req = 'SELECT hs.`post_id`
                FROM `'._DB_PREFIX_.'gdz_blog_posts` hs
    ...
    public function getPosts($category_id = 0, $state = -1, $start = 0, $limit = 20)
    {
        ...
        $sql = '
            ...
            LEFT JOIN '._DB_PREFIX_.'gdz_blog_categories_lang hssll ON (hss.`category_id` = hssll.`category_id`)
    ...
    public function getPostCount($category_id = 0, $state = -1)
    {
        ...
        $sql = '
            ...
            FROM '._DB_PREFIX_.'gdz_blog_posts hss
            INNER JOIN '._DB_PREFIX_.'gdz_blog_posts_lang hssl ON (hss.`post_id` = hssl.`post_id`)
            INNER JOIN '._DB_PREFIX_.'gdz_blog_categories_lang hssll ON (hss.`category_id` = hssll.`category_id` AND hss.`category_id` IS NOT NULL)
    ...
    public function renderNavigation()
    {
        ...
        $html .= '<a class="btn btn-default" href="'.AdminController::$currentIndex.
            '&configure='.$this->name.'
                &token='.Tools::getAdminTokenLite('AdminGdzblogPost').'" title="Back to Dashboard"><i class="icon-home"></i>Back to Dashboard</a>';
        $html .= '</div>';
        return $html;
    }
    ...
    public function renderAddPost()
    {
        ...
        $this->tpl_folder = 'gdz_blogform/';
        $this->tpl_form_vars = array(
            'base_url' => $this->root_url,
            'languages' => $this->context->controller->getLanguages(),
            'id_language' => $this->context->language->id,
            'image_baseurl' => $this->root_url.'modules/gdz_blog/views/img/'
        );
    ...
    {
        if (Tools::getValue('controller') != 'AdminGdzblogPost' && Tools::getValue('configure') != $this->name) {
            return;
        }
