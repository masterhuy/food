...
class AdminGdzblogCommentController extends ModuleAdminController
{
    ...
    private function _postProcess()
    {
        if (Tools::isSubmit('delete_id_comment')) {
            $item = new gdzComment((int)Tools::getValue('delete_id_comment'));
            $res = $item->delete();
            if (!$res) {
                $this->_html .= Tools::displayError('Could not delete');
            } else {
                Tools::redirectAdmin($this->context->link->getAdminLink('AdminGdzblogComment', true).'&conf=1&configure='.$this->name.'&tab_module='.$this->tab.'&module_name='.$this->name);
            }
        } elseif (Tools::isSubmit('changeCommentStatus') && Tools::isSubmit('status_id_comment')) {
            $item = new gdzComment((int)Tools::getValue('status_id_comment'));
            if ($item->status == 0) {
                $item->status = 1;
            } else {
                $item->status = 0;
            ...
            if (!$res) {
                ...
                Tools::redirectAdmin($this->context->link->getAdminLink('AdminGdzblogComment', true).'&conf=5&configure='.$this->name.'&tab_module='.$this->tab.'&module_name='.$this->name);
            }
            ...
            $res = Db::getInstance()->execute('UPDATE `'._DB_PREFIX_.'gdz_blog_posts_comments` SET status = 1 WHERE status = -2');
            if (!$res) {
                $this->_html .= Tools::displayError('The comment cant approved.');
            } else {
                Tools::redirectAdmin($this->context->link->getAdminLink('AdminGdzblogComment', true).'&conf=5&configure='.$this->name.'&tab_module='.$this->tab.'&module_name='.$this->name);
    ...
    public function commentExists($id_comment)
    {
        $req = 'SELECT hs.`post_id`
                FROM `'._DB_PREFIX_.'gdz_blog_posts_comments` hs
    ...
    public function getComments($post_id = 0, $state = -1, $start = 0, $limit = 20)
    {
        ...
        $sql = '
            ...
            FROM '._DB_PREFIX_.'gdz_blog_posts_comments hss
            WHERE 1 '.
            $filter.
    ...
    public function renderNavigation()
    {
        ...
        $html .= '<a class="btn btn-default" href="'.AdminController::$currentIndex.
            '&configure='.$this->name.'
                &token='.Tools::getAdminTokenLite('AdminGdzblogComment').'" title="Back to Dashboard"><i class="icon-home"></i>Back to Dashboard</a>';
        $html .= '</div>';
        return $html;
    }
