...
class AdminGdzmegamenuStyleController extends ModuleAdminController
{
    ...
    public function initPageHeaderToolbar()
    {
        ...
        $this->page_header_toolbar_btn['save_style'] = array(
              'href' => $this->context->link->getAdminLink('AdminGdzmegamenuStyle').'&saveStyle&menu_id='.$this->selected_menu['menu_id'],
              'desc' => $this->l('Save Menu Style', null, null, false),
              'icon' => 'process-icon-save'
        );
        $this->page_header_toolbar_btn['reset_style'] = array(
              'href' => $this->context->link->getAdminLink('AdminGdzmegamenuStyle').'&resetStyle&menu_id='.$this->selected_menu['menu_id'],
    ...
    private function _postProcess()
    {
        ...
        if (count($errors)) {
            ...
            Tools::redirectAdmin($this->context->link->getAdminLink('AdminGdzmegamenuStyle', true).'&conf=4&configure='.$this->name.'&tab_module='.$this->tab.'&module_name='.$this->name);
        } elseif (Tools::isSubmit('selectMenu')) {
            Tools::redirectAdmin($this->context->link->getAdminLink('AdminGdzmegamenuStyle', true).'&configure='.$this->name.'&tab_module='.$this->tab.'&module_name='.$this->name.'&menu_id='.$this->selected_menu['menu_id']);
        }
    }
    ...
    {
        if (Tools::getValue('controller') != 'AdminGdzmegamenuStyle' && Tools::getValue('configure') != $this->name) {
            return;
        }
    ...
    {
        $this->menu .= '<div id="gdz-megamenu-container" class="navbar clearfix"><div class="gdz-megamenu"><ul class="nav level0">';
    }
