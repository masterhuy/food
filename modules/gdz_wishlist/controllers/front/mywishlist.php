...
class gdz_wishlistMyWishListModuleFrontController extends ModuleFrontController
{
	public $ssl = true;
	...
	public function initContent()
	{
		...
		$this->context->controller->addCSS(_MODULE_DIR_.'gdz_wishlist/views/css/style.css');
	...
	public function assign()
	{
		...
		if ($this->context->customer->isLogged())
		{
				...
				if (!count($errors))
				{
					...
					if (!count($errors))
					{
						...
						/*Mail::Send(
							...
							'{message}' => $this->context->link->getModuleLink('gdz_wishlist', 'view', array('token' => $wishlist->token))
							),
						...
						Tools::redirect($this->context->link->getModuleLink('gdz_wishlist', 'mywishlist'));
					}
				}
			}
			else if ($add)
			...
			Tools::redirect('index.php?controller=authentication&back='.urlencode($this->context->link->getModuleLink('gdz_wishlist', 'mywishlist')));
		...
		$this->setTemplate('module:gdz_wishlist/views/templates/front/mywishlist.tpl');
	}
