		...
		<div class="item">
			{foreach from = $posts_slide item = post}
				...
				<div class="blog-item">
						...
						<div class="post-thumb">
							<a href="#"><img src="{$image_url nofilter}resized_{$post.image nofilter}" alt="{$post.title nofilter}" class="img-responsive" /></a>
						</div>
						{/if}
						<ul class="post-meta">
							{if $show_category == '1'}
							<li class="post-category"><a href="{$post.catlink|escape:'htmlall':'UTF-8'|replace:'&amp;':'&'}">{$post.category_name nofilter}</a></li>
							{/if}
							{if $show_time == '1'}
							<li class="post-created">{$post.created|escape:'html':'UTF-8'|date_format:'%B %e, %Y'}</li>
							{/if}
							{if $show_nviews == '1'}
							<li class="post-views"> - {$post.views nofilter} Views</li>
							{/if}
							{if $show_ncomments == '1'}
							<li class="post-comments"> - {$post.comment_count nofilter} Comments</li>
							{/if}
						</ul>
						<h4 class="post-title"><a href="{$post.postlink|escape:'htmlall':'UTF-8'|replace:'&amp;':'&'}">{$post.title nofilter}</a></h4>
						{if $show_introtext == '1'}
						<div class="post-intro">{$post.introtext nofilter}</div>
						{/if}
						{if $show_readmore == '1'}
							<a class="post-readmore" href="{$post.postlink|escape:'htmlall':'UTF-8'|replace:'&amp;':'&'}"><span>{$readmore_text nofilter}</span></a>
						{/if}
				</div>
			{/foreach}
		</div>
